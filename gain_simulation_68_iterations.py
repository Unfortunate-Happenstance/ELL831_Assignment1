...
def optimize():
    ...
    all_results = []  
    threads = []
    for j in range(0, len(W_range)//8):
    ...
    gain_values = [entry[6] for entry in results]  # gain magnitude values
    max_gain_index = max(range(len(gain_values)), key=lambda i: gain_values[i])

    print("[RESULT] Max gain:", results[max_gain_index][6], "dB")
    return results


def save_results_to_json(results_full, filename="gain_simulation_results_68_iterations.json"):
